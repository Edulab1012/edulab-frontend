...
export default function EditProfile({ initialData, onSave, onCancel }: EditProfileProps) {
  ...
  const handleImageUpload = async (file: File, type: "avatar" | "background") => {
    ...
    try {
      ...
      if (type === "avatar") {
        imageUrl = await uploadCloudinary(file, "image")
      } else {
        imageUrl = await uploadBgCloudinary(file, "image")
      }
      ...
      <Card className="shadow-2xl border-0 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm">
        ...
        <div className="relative h-40 sm:h-60 rounded-t-lg overflow-hidden">
          ...
          <Button
            onClick={onCancel}
            variant="outline"
            className="absolute right-2 top-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm border-white/20 text-white"
          >
            ...
            <div className="relative group w-fit">
              ...
              <div onClick={() => avatarInputRef.current?.click()} className="cursor-pointer">
                <Avatar className="h-32 w-32 border-4 border-white dark:border-slate-700 shadow-xl transition-all hover:opacity-80">
                  <AvatarImage src={formData.avatarUrl || "/placeholder.svg?height=128&width=128"} alt="Profile" />
              ...
              {socialFields.map((field) => (
                <div key={field.name} className="space-y-2">
                  ...
                  <Input
                    id={field.name}
                    name={field.name}
                    value={formData.socials?.[field.name as keyof typeof formData.socials] || ""}
